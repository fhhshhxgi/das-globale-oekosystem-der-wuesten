    ...
    >
      ...
      <div className="max-w-7xl mx-auto space-y-10 relative z-10 w-full">
        ...
        <div className="text-center space-y-3">
          ...
          <h2 className="font-sans text-2xl md:text-4xl font-black uppercase text-white tracking-tight leading-none">
                ...
                <div className="bg-[#090a15] rounded-2xl border border-white/5 p-4 md:p-5 shadow-inner space-y-4 relative overflow-hidden">
                  ...
                  {activeInt.id === 'innerartlich' && (
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center">
                        {/* Interactive SVG map representation */}
                        <div className="sm:col-span-6 h-32 bg-slate-950 rounded-xl border border-white/5 overflow-hidden relative flex items-center justify-center p-2">
                          <svg className="w-full h-full" viewBox="0 0 200 120">
                            {/* Territory grid background */}
                            <defs>
                              <pattern id="dot-grid" width="10" height="10" patternUnits="userSpaceOnUse">
                                <circle cx="1.5" cy="1.5" r="1" fill="rgba(255,255,255,0.02)" />
                              </pattern>
                            </defs>
                            <rect width="200" height="120" fill="url(#dot-grid)" />
                            
                            {/* Render overlapping circles depending on density */}
                            {fennekPositions.slice(0, fennekDensity).map((pos, idx) => (
                              <ellipse
                                key={idx}
                                cx={pos.cx}
                                cy={pos.cy}
                                rx={pos.rx - fennekDensity * 1.2}
                                ry={pos.ry - fennekDensity * 1.2}
                                className={`stroke-[1.5] fill-amber-500/5 transition-all duration-500 ${pos.color}`}
                                strokeDasharray="3 2"
                              />
                            ))}
                            
                            {/* Warning overlay for heavy overlap */}
                            {fennekDensity >= 4 && (
                              <rect x="0" y="0" width="200" height="120" fill="rgba(239, 68, 68, 0.04)" className="animate-pulse pointer-events-none" />
                            )}
                          </svg>
                          <div className="absolute bottom-2 right-2 px-1.5 py-0.5 rounded text-[8px] font-mono bg-slate-900/90 border border-white/10 text-slate-400">
                            Fennek Revier-Kompression
                          </div>
                        </div>

                        {/* Interactive dynamic readouts */}
                        <div className="sm:col-span-6 space-y-2 text-[10px] font-mono">
                          <div className="p-2 bg-slate-950 rounded-lg border border-white/5 flex justify-between">
                            <span className="text-slate-400">Reviergröße pro Fuchs:</span>
                            <span className="text-amber-300 font-bold">{territorySize} km²</span>
                          </div>
                          <div className="p-2 bg-slate-950 rounded-lg border border-white/5 space-y-1">
                            <div className="flex justify-between">
                              <span className="text-slate-400 font-bold">Verhaltensstress-Pegel:</span>
                  ...
                  {activeInt.id === 'zwischenartlich' && (
                    <div className="space-y-4">
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-[10px] font-mono">
                          <span className="text-slate-400 font-bold">REICHTUM SAMENAUFKOMMEN (NAHRUNG):</span>
                          <span className="text-orange-400 font-bold">
                            {seedAbundance < 30 ? '🏜️ Extremes Dürrejahr' : seedAbundance > 70 ? '🌱 Sintflutartiger Wüstenregen' : '🌤️ Normales Trockenjahr'} ({seedAbundance}%)
                          </span>
                        </div>
                        <input
                          type="range"
                      ...
                      <div className="flex flex-wrap gap-2 justify-center border-t border-white/5 pt-3">
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-xl text-[9px] font-mono font-bold border transition-all cursor-pointer ${showMouse ? 'bg-amber-500/10 border-amber-500/50 text-amber-300' : 'bg-slate-950 border-white/5 opacity-40 text-slate-500'}`}
                        >
                          🦊 Wüstenrennmaus
                        </button>
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-xl text-[9px] font-mono font-bold border transition-all cursor-pointer ${showAnts ? 'bg-rose-500/10 border-rose-500/50 text-rose-300' : 'bg-slate-950 border-white/5 opacity-40 text-slate-500'}`}
                        >
                          🐜 Ernteameise
                        </button>
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-xl text-[9px] font-mono font-bold border transition-all cursor-pointer ${showBeetles ? 'bg-purple-500/10 border-purple-500/50 text-purple-300' : 'bg-slate-950 border-white/5 opacity-40 text-slate-500'}`}
                        >
                          🪲 Schwarzkäfer
                        </button>
                      </div>

                      {/* Displaying share values */}
                      <div className="space-y-2.5">
                        <span className="text-[9px] font-mono font-bold text-slate-400 block uppercase">RESSOURCENVERTEILUNG (Nahrungsanteile):</span>
                        
                        {/* Mouse bar */}
                        {showMouse && (
                          <div className="space-y-1">
                            <div className="flex justify-between text-[9px] font-mono">
                              <span className="text-amber-300 font-bold">Wüstenrennmaus (Säugetier)</span>
                              <span className="text-slate-400">{mouseShare}% (Energieaufwand hoch)</span>
                            </div>
                            <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden border border-white/5">
                              <div className="bg-amber-500 h-full transition-all duration-300" style={{ width: `${mouseShare}%` }} />
                            </div>
                          </div>
                        )}
                        ...
                        {showAnts && (
                          <div className="space-y-1">
                            <div className="flex justify-between text-[9px] font-mono">
                              <span className="text-rose-300 font-bold">Ernteameise (Kleininsekt)</span>
                              <span className="text-slate-400">{antShare}% (Spezialist für Kleinstsamen)</span>
                            </div>
                            <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden border border-white/5">
                              <div className="bg-rose-500 h-full transition-all duration-300" style={{ width: `${antShare}%` }} />
                            </div>
                          </div>
                        )}
                        ...
                        {showBeetles && (
                          <div className="space-y-1">
                            <div className="flex justify-between text-[9px] font-mono">
                              <span className="text-purple-300 font-bold">Schwarzkäfer (Chitinpanzer)</span>
                              <span className="text-slate-400">
                                {beetleShare}% {beetleShare < 15 && '⚠️ Stark verdrängt'}
                              </span>
                            </div>
                            <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden border border-white/5">
                              <div className="bg-purple-500 h-full transition-all duration-300" style={{ width: `${beetleShare}%` }} />
                            </div>
                          </div>
                        )}
                        ...
                        <div className="p-2.5 bg-slate-950 rounded-xl border border-white/5 text-[9px] font-mono text-slate-400 leading-normal">
                          {mouseShare > 0 && antShare > 0 && beetleShare > 0 && seedAbundance < 30 ? (
                            <p className="text-rose-400 font-bold">
                              ⚠️ KONKURRENZAUSSCHLUSS-REGEL: Bei extremer Dürre verdrängen Nagetiere und Ameisen den Käfer fast gänzlich. Der Käfer muss in zeitlich andere ökologische Nischen (z.B. absolute Nachtaktivität) ausweichen!
                            </p>
                          ) : (
                            <p>
                              ℹ️ NISCHENDIFFERENZIERUNG: Durch die Spezialisierung auf unterschiedliche Samengrößen und Aktivitätszyklen können alle drei Organismengruppen parallel im Biotop überleben.
                            </p>
                          )}
                        </div>
                      </div>
                  ...
                  {activeInt.id === 'symbiose' && (
                    <div className="space-y-4">
                      ...
                      {/* Central Animated Interchange Station */}
                      <div className="p-3 bg-slate-950 rounded-xl border border-white/5 flex flex-col items-center gap-3 relative overflow-hidden">
                        
                        <div className="flex justify-between w-full text-[9px] font-mono">
                          <span className="text-emerald-400 font-bold">🌳 Akazie (Blätter)</span>
                          <span className="text-cyan-400 font-bold">🍄 Mykorrhiza (Fasern)</span>
                        </div>

                        {/* Animated Transfer visualization */}
                        <div className="w-full h-8 bg-slate-900 rounded-lg relative border border-white/10 flex items-center justify-between px-6">
                          <span className="text-sm">🌳</span>
                          
                          {/* Animated flow dots */}
                          <div className="flex-1 relative h-full overflow-hidden flex items-center justify-center">
                            {transferState === 'transferring' ? (
                              <div className="absolute inset-0 flex items-center justify-between">
                                {/* Yellow sugar moving right */}
                                <motion.div
                                  initial={{ x: -20, opacity: 0 }}
                                  animate={{ x: 100, opacity: [0, 1, 1, 0] }}
                                  transition={{ duration: 1.2, repeat: 1 }}
                                  className="w-1.5 h-1.5 rounded-full bg-yellow-400 absolute"
                                />
                                {/* Blue minerals moving left */}
                                <motion.div
                              ...
                              <span className="text-[9px] font-mono text-slate-500 italic">Bereit zum Transfer</span>
                            )}
                          </div>
                        ...
                        <div className="flex justify-between items-center w-full">
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg border text-[9px] font-mono font-bold transition-all cursor-pointer flex items-center gap-1.5 ${transferState === 'transferring' ? 'bg-slate-900 border-white/5 text-slate-500' : 'bg-gradient-to-r from-emerald-600 to-cyan-600 border-emerald-500 text-white hover:brightness-110'}`}
                          >
                            <RefreshCw className="w-3 h-3" />
                            Nährstoff-Austausch aktivieren
                          </button>
                          
                          <div className="text-right text-[9px] font-mono">
                            <span className="text-slate-400">Gemeinsamer Überlebensbonus: </span>
                            <span className="text-emerald-300 font-black">+{mutualFitness}%</span>
                          </div>
                        </div>
                      </div>
                    </div>
